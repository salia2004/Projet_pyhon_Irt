    ...
    while i < len(text): 
        ...
        if text[i] == '<':
            ...
            while j < len(text) and text[j] != '>':
                balise += text[j]
                j += 1
            # Vérifier si la balise extraite est dans la liste des balises HTML connues
            if balise in liste_balise :  # si l'élément est bien une balise, on l'ajoute à la liste
                liste.append(balise)
...
def verifier_balises(text_html):
    ...
    for balise in liste_balise:
        ...
        if (not balise.startswith('/')) and (balise not in balise_auto_fermante):#on ne prend pa en compte les balise auto fermante qui n'apparaise que une fois 
            pile_balise.append(balise)
        else:
            b_fermant=balise[1:]
            if not pile_balise or pile_balise[-1]!=b_fermant:
                return False
            pile_balise.pop()
...
def compter_balises_p(fichier_html):
    ...
    try:
        ...
        if verifier_balises(texte):
            ...
            print("Dossier html non equilibree")
            return {}
    except FileNotFoundError:
        print(f"Erreur : Le fichier {fichier_html} est introuvable.")
        return {}
